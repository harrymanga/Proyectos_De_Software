...
local function parseAddress(input)
    ...
    if type(input) ~= "string" then return nil, "la dirección debe ser una cadena o un número" end
    local addr = getAddressSafe(input)
    if not addr or addr == 0 then return nil, "Dirección no válida: " .. tostring(input) end
    return addr, nil
end
...
local function requireProcess()
    ...
    if (getOpenedProcessID() or 0) == 0 then
        return { success = false, error = "No hay proceso adjunto", error_code = "NO_PROCESS" }
    end
    return nil
end
...
local function cleanupZombieState()
    log("Limpiando recursos zombies...")
    ...
    if cleaned.breakpoints > 0 or cleaned.dbvm_watches > 0 or cleaned.scans > 0 then
        log(string.format("Limpiado: %d breakpoints, %d DBVM watches, %d scans",
...
local function encode_table(val, stack)
  ...
  if stack[val] then error("referencia circular") end
  stack[val] = true
...
local function decode_string(str, pos)
  ...
  while true do
    ...
    if not endpos then return nil, "cotización de cierre esperada" end
...
local function decode_number(str, pos)
  ...
  if not val then return nil, "número inválido" end
...
local function decode_array(str, pos)
  ...
  while true do
    ...
    if c ~= "," then return nil, "esperado ']' o ','" end
...
local function decode_object(str, pos)
  ...
  while true do
        ...
        if not key then return nil, "clave de cadena esperada" end
    pos = decode_scanwhite(str, pos)
    if str:sub(pos, pos) ~= ":" then return nil, "esperado ':'" end
    pos = decode_scanwhite(str, pos + 1)
        local val
        val, pos = decode(str, pos)
        obj[key] = val
...
function cmd_get_process_info(params)
    ...
    if pid and pid > 0 then
        ...
        if #moduleList == 0 then
            return {
                success = false,
                error = "Proceso adjunto pero no puede enumerar módulos (probablemente interferencia anti-trampas). Pruebe enum_modules directamente o adjunte a un proceso diferente.",
    ...
    return { success = false, error = "No hay proceso adjunto" }
end
...
function cmd_get_symbol_address(params)
    ...
    if not symbol then return { success = false, error = "Sin nombre de símbolo" } end
    
    local addr = getAddressSafe(symbol)
    if addr then
        return { success = true, symbol = symbol, address = toHex(addr), value = addr }
    end
    return { success = false, error = "Símbolo no encontrado: " .. symbol }
end
...
function cmd_read_memory(params)
    ...
    if not addr then return { success = false, error = "Dirección no válida" } end
    
    local bytes = readBytes(addr, size, true)
    if not bytes then return { success = false, error = "No se pudo leer en " .. toHex(addr) } end
...
function cmd_read_integer(params)
    ...
    else return { success = false, error = "Tipo desconocido: " .. tostring(itype) } end
    
    if val == nil then return { success = false, error = "No se pudo leer en " .. toHex(addr) } end
...
function cmd_read_pointer(params)
    ...
    if not base then return { success = false, error = "Dirección base no válida" } end
    ...
    for i, offset in ipairs(offsets) do
        ...
        if not ptr then
            return { success = false, error = "No se pudo leer el puntero en " .. toHex(currentAddr), path = path }
        end
...
function cmd_aob_scan(params)
    ...
    if not pattern then return { success = false, error = "No se proporciona ningún patrón" } end
...
function cmd_get_scan_results(params)
    ...
    if not serverState.scan_foundlist then
        return { success = false, error = "No hay resultados de escaneo. Ejecute scan_all primero." }
    end
...
function cmd_next_scan(params)
    ...
    if not serverState.scan_memscan then
        return { success = false, error = "Sin escaneo previo. Ejecute scan_all primero." }
    end
...
function cmd_write_integer(params)
    ...
    if vtype == "byte" then
        if type(value) ~= "number" or value < 0 or value > 0xFF then
            return { success = false, error = "Valor demasiado grande para el tipo", error_code = "INVALID_PARAMS" }
        end
    ...
    elseif vtype == "double" then
        ...
        return { success = false, error = "Tipo desconocido: " .. tostring(vtype) }
    end

    if not ok then
        return { success = false, error = "Error de escritura: " .. tostring(err), address = toHex(addr) }
    end
...
function cmd_write_memory(params)
    ...
    if type(addr) == "string" then addr = getAddressSafe(addr) end
    if not addr then return { success = false, error = "Dirección no válida" } end
    if not bytes or #bytes == 0 then return { success = false, error = "No se proporcionan bytes" } end
...
function cmd_write_string(params)
    ...
    if type(addr) == "string" then addr = getAddressSafe(addr) end
    if not addr then return { success = false, error = "Dirección no válida" } end
    if not str then return { success = false, error = "No se proporciona ninguna cadena" } end
...
function cmd_get_instruction_info(params)
    ...
    if not ok or not disasm then
        return { success = false, error = "No se pudo disassemble en " .. toHex(addr) }
    end
...
function cmd_find_function_boundaries(params)
    ...
    return {
        ...
        note = not found and "No se encontró ningún prólogo de función estándar dentro del rango de búsqueda" or nil
    }
end
...
function cmd_analyze_function(params)
    ...
    if not funcStart then 
        return { 
            success = false, 
            error = "No se pudo encontrar el inicio de la función",
            arch = is64 and "x64" or "x86",
            query_address = toHex(addr)
        } 
    end
...
function cmd_find_references(params)
    ...
    if not targetAddr then return { success = false, error = "Dirección no válida" } end
...
function cmd_find_call_references(params)
    ...
    if not funcAddr then return { success = false, error = "Dirección de función no válida" } end
...
function cmd_set_breakpoint(params)
    ...
    if not slot then
        return { success = false, error = "No hay ranuras libres para puntos de interrupción de hardware (máximo 4 registros de depuración)." }
    end
...
function cmd_remove_breakpoint(params)
    ...
    return { success = false, error = "Punto de interrupción no encontrado: " .. tostring(bpId) }
end
...
function cmd_evaluate_lua(params)
    ...
    if not code then return { success = false, error = "No se proporcionó ningún código" } end
    
    local fn, err = load(code, "mcp_evaluate_lua", "t")
    if not fn then return { success = false, error = "Error de compilación: " .. tostring(err) } end
    
    local ok, result = pcall(fn)
    if not ok then return { success = false, error = "Error de tiempo de ejecución: " .. tostring(result) } end
...
function cmd_create_thread(params)
    ...
    local ok, err = pcall(function()
        createThread(function(thread, a)
            ...
            if not f then error("Error de compilación: " .. tostring(ferr)) end
            return f(thread, a)
        end, arg)
    end)

    if not ok then
        return { success = false, error = "createThread fallido: " .. tostring(err) }
    end
...
function cmd_get_global_variable(params)
    ...
    if not name then return { success = false, error = "No se proporcionó ningún nombre de variable" } end

    local ok, value = pcall(getGlobalVariable, name)
    if not ok then
        return { success = false, error = "getGlobalVariable fallido: " .. tostring(value) }
    end
...
function cmd_set_global_variable(params)
    ...
    local value = params.value
    if not name  then return { success = false, error = "No se proporcionó ningún nombre de variable"  } end
    if value == nil then return { success = false, error = "No se proporcionó ningún valor" } end

    local ok, err = pcall(setGlobalVariable, name, value)
    if not ok then
        return { success = false, error = "setGlobalVariable fallido: " .. tostring(err) }
    end
    return { success = true }
end
...
function cmd_queue_to_main_thread(params)
    ...
    if not ok then
        return { success = false, error = "queue fallido: " .. tostring(err) }
    end
    return { success = true }
end
...
function cmd_check_synchronize(params)
    ...
    if not ok then
        return { success = false, error = "checkSynchronize fallido: " .. tostring(err) }
    end
    return { success = true }
end
...
function cmd_in_main_thread(params)
    ...
    if not ok then
        return { success = false, error = "inMainThread fallido: " .. tostring(result) }
    end
...
function cmd_ping(params)
    return {
        ...
        message = "CE MCP Bridge v" .. VERSION .. " vivo"
    }
end
...
function cmd_search_string(params)
    ...
    if not searchStr then return { success = false, error = "Sin cadena de búsqueda" } end
...
function cmd_dissect_structure(params)
    ...
    if not address then return { success = false, error = "Dirección no válida" } end
    ...
    if not ok or not struct then
        return { success = false, error = "No se pudo crear la estructura" }
    end
...
function cmd_auto_assemble(params)
    ...
    if not script then return { success = false, error = "No se proporciona ningún script" } end
    ...
    if success then
        ...
        return {
            success = false,
            error = "AutoAssemble fallido: " .. tostring(disableInfo)
        }
    end
end
...
function cmd_enum_memory_regions_full(params)
    ...
    if not ok or not regions then
        return { success = false, error = "enumMemoryRegions fallido" }
    end
...
function cmd_read_pointer_chain(params)
    ...
    for i, offset in ipairs(offsets) do
        ...
        if not ptr then
            return {
                success = false,
                error = "No se pudo leer el puntero en el paso " .. i,
                partial_chain = chain,
...
function cmd_get_rtti_classname(params)
    ...
    if className then
        ...
        return {
            ...
            class_name = nil,
            found = false,
            note = "No se encontró información RTTI en esta dirección"
        }
    end
end
...
function cmd_checksum_memory(params)
    ...
    if ok and hash then
        ...
        return {
            ...
            address = toHex(address),
            size = size,
            error = "No se pudo calcular MD5: " .. tostring(hash)
        }
    end
end
...
function cmd_generate_signature(params)
    ...
    if not ok then
        return {
            success = false,
            address = toHex(addr),
            error = "getUniqueAOB fallido: " .. tostring(signature)
        }
    end
    ...
    if not signature or signature == "" then
        return {
            success = false,
            address = toHex(addr),
            error = "No se pudo generar una firma única: el patrón no es lo suficientemente único"
        }
    end
    ...
    return {
        ...
        usage_hint = string.format("aob_scan('%s') luego agrega offset %d para alcanzar el objetivo", signature, offset or 0)
    }
end
...
function cmd_get_physical_address(params)
    ...
    if not ok then
        return {
            ...
            error = "Controlador DBK no cargado. Ejecute dbk_initialize() primero o cárguelo a través de la configuración CE."
        }
    end
    ...
    if not phys or phys == 0 then
        return {
            success = false,
            virtual_address = toHex(addr),
            error = "No se pudo resolver la dirección física. Es posible que la página no esté presente en la RAM."
        }
    end
...
function cmd_start_dbvm_watch(params)
    ...
    if not dbk_initialized() then
        return { success = false, error = "Controlador DBK no cargado. Ir a Settings -> Debugger -> Kernelmode" }
    end
    ...
    if not dbvm_initialized() then
        ...
        if not dbvm_initialized() then
            return { success = false, error = "DBVM no se ejecuta. Ir a Settings -> Debugger -> Use DBVM" }
        end
    end
    ...
    if not ok or not phys or phys == 0 then
        return {
            success = false,
            virtual_address = toHex(addr),
            error = "No se pudo resolver la dirección física. Es posible que la página esté omitida o no sea válida."
        }
    end
    ...
    if serverState.active_watches[watchKey] then
        return {
            success = false,
            virtual_address = toHex(addr),
            error = "Ya estoy viendo esta dirección. Llame primero a stop_dbvm_watch."
        }
    end
    ...
    log(string.format("Iniciando la observación de DBVM en Phys: 0x%X (Modo: %s)", phys, mode))

    if mode == "x" then
        if not dbvm_watch_executes then
            return { success = false, error = "Falta la función dbvm_watch_executes en el motor CE Lua" }
        end
    ...
    if not okWatch then
        return {
            ...
            virtual_address = toHex(addr),
            physical_address = toHex(phys),
            error = "Observación DBVM ROTO/FALLADO: " .. tostring(result)
        }
    end
    ...
    if not watch_id then
        return {
            ...
            virtual_address = toHex(addr),
            physical_address = toHex(phys),
            error = "La observación DBVM devolvió nulo (consulte la consola CE para obtener más detalles)"
        }
    end
    ...
    return {
        ...
        watch_id = watch_id,
        mode = mode,
        note = "Llame a poll_dbvm_watch para obtener registros sin detenerse, o stop_dbvm_watch para finalizar"
    }
end
...
function cmd_poll_dbvm_watch(params)
    ...
    if not watchInfo then
        return {
            success = false,
            virtual_address = toHex(addr),
            error = "No se encontró ningún observador activo para esta dirección. Llame primero a start_dbvm_watch."
        }
    end
    ...
    return {
        ...
        hit_count = #results,
        hits = results,
        note = "observador todavía activo. Llame nuevamente para obtener más registros o stop_dbvm_watch para finalizar."
    }
end
...
function cmd_stop_dbvm_watch(params)
    ...
    if not watchInfo then
        return {
            success = false,
            virtual_address = toHex(addr),
            error = "No se encontró ningún observador activo para esta dirección"
        }
    end
    ...
    return {
        ...
        hits = results,
        note = #results > 0 and "Instrucciones encontradas que accedieron a la memoria." or "No se detectaron accesos durante el seguimiento"
    }
end
...
function cmd_output_debug_string(params)
    ...
    if type(message) ~= "string" then
        return { success = false, error = "el mensaje debe ser una cadena", error_code = "INVALID_PARAMS" }
    end
...
function cmd_speak_text(params)
    ...
    if type(text) ~= "string" then
        return { success = false, error = "el texto debe ser una cadena", error_code = "INVALID_PARAMS" }
    end
    local ok, err
...
function cmd_play_sound(params)
    if type(params.filename) ~= "string" or params.filename:find("%.%.") then
        return { success = false, error = "Nombre de archivo no válido", error_code = "INVALID_PARAMS" }
    end
...
function cmd_set_progress_state(params)
    ...
    if not tbState then
        return { success = false, error = "El estado debe ser uno de: none, normal, paused, error, indeterminate", error_code = "INVALID_PARAMS" }
    end
...
function cmd_set_progress_value(params)
    ...
    if type(current) ~= "number" or type(max) ~= "number" then
        return { success = false, error = "Actual y máximo deben ser números", error_code = "INVALID_PARAMS" }
    end
...
local function dbkNotLoadedError()
    return {
        success = false,
        error = "El controlador kernel (DBK) o el hipervisor (DBVM) no están cargados",
...
function cmd_read_process_memory_cr3(params)
    ...
    if not cr3_str or not addr_str or not size or size <= 0 then
        return { success = false, error = "Los parámetros cr3, dirección y tamaño son obligatorios." }
    end
    ...
    if not cr3 or not addr then
        return { success = false, error = "CR3 o valor de dirección no válido" }
    end

    local ok, byteTable = pcall(readProcessMemoryCR3, cr3, addr, size)
    if not ok then return dbkNotLoadedError() end
    if not byteTable then
        return { success = false, error = "Error de lectura: la página puede estar paginada o no ser válida" }
    end
...
function cmd_write_process_memory_cr3(params)
    ...
    if not cr3_str or not addr_str or not bytes or type(bytes) ~= "table" then
        return { success = false, error = "Los parámetros cr3, dirección y bytes (lista) son obligatorios" }
    end
...
function cmd_map_memory(params)
    ...
    if not addr_str or not size or size <= 0 then
        return { success = false, error = "La dirección y el tamaño de los parámetros son obligatorios." }
    end

    local addr = type(addr_str) == "string" and getAddressSafe(addr_str) or addr_str
    if not addr then
        return { success = false, error = "Valor de dirección no válido" }
    end

    local ok, mappedAddr, mdl = pcall(mapMemory, addr, size)
    if not ok then return dbkNotLoadedError() end
    if not mappedAddr then
        return { success = false, error = "mapMemory fallido: la dirección puede no ser válida o el DBK no está cargado" }
    end
...
function cmd_unmap_memory(params)
    ...
    if not addr_str then
        return { success = false, error = "El parámetro mapped_address es obligatorio" }
    end

    local addr = type(addr_str) == "string" and getAddressSafe(addr_str) or addr_str
    if not addr then
        return { success = false, error = "Valor de mapped_address no válido" }
    end
...
function cmd_dbk_writes_ignore_write_protection(params)
    ...
    if type(enable) ~= "boolean" then
        return { success = false, error = "Parámetro enable (boolean) es requerido" }
    end
...
function cmd_get_physical_address_cr3(params)
    ...
    if not cr3_str or not va_str then
        return { success = false, error = "Los parámetros cr3 y virtual_address son obligatorios" }
    end

    local cr3 = type(cr3_str) == "string" and getAddressSafe(cr3_str) or cr3_str
    local va  = type(va_str)  == "string" and getAddressSafe(va_str)  or va_str
    if not cr3 or not va then
        return { success = false, error = "Valor cr3 o virtual_address no válido" }
    end

    local ok, phys = pcall(getPhysicalAddressCR3, cr3, va)
    if not ok then return dbkNotLoadedError() end
    if not phys then
        return { success = false, error = "Dirección no paginada: es posible que la dirección virtual no esté asignada en este CR3" }
    end
...
local function sanitizeFilename(f)
    if type(f) ~= "string" or f == "" then return nil, "Nombre de archivo no válido" end
    if f:find("%.%.") then return nil, "No se permite el recorrido de la ruta" end
    return f, nil
end
...
function cmd_get_file_version(params)
    ...
    if type(verTable) ~= "table" then
        return { success = false, error = "getFileVersion no devolvió una tabla de versiones" }
    end
...
function cmd_write_clipboard(params)
    local text = params.text
    if type(text) ~= "string" then return { success = false, error = "El texto debe ser una cadena" } end
...
function cmd_run_command(params)
    ...
    if not command or command == "" then
        return { success = false, error = "No se proporciona ningún comando" }
    end

    local ok, output, exitCode = pcall(runCommand, command, args)

    if not ok then
        return { success = false, error = "runCommand fallido: " .. tostring(output) }
    end
...
function cmd_shell_execute(params)
    ...
    if verb ~= "open" then
        return { success = false, error = "Verbo no compatible para shell_execute: " .. tostring(verb), error_code = "INVALID_PARAMS" }
    end

    if showCommand ~= nil and type(showCommand) ~= "number" then
        return { success = false, error = "showcommand debe ser un número cuando se proporciona", error_code = "INVALID_PARAMS" }
    end

    local ok, err = pcall(shellExecute, command, args, workingDir ~= "" and workingDir or nil, showCommand)

    if not ok then
        return { success = false, error = "shellExecute fallido: " .. tostring(err) }
    end
...
local function resolveStructure(params)
    ...
    if not sid then
        return nil, { success = false, error = "structure_id es requerido", error_code = "INVALID_PARAMS" }
    end
    local structure = serverState.structures[sid]
    if not structure then
        return nil, { success = false, error = "Desconocido structure_id: " .. tostring(sid), error_code = "NOT_FOUND" }
    end
...
function cmd_create_structure(params)
    ...
    if not name or name == "" then
        return { success = false, error = "El nombre es requerido", error_code = "INVALID_PARAMS" }
    end

    local ok, structure = pcall(createStructure, name)
    if not ok or not structure then
        return { success = false, error = "createStructure fallido: " .. tostring(structure), error_code = "CE_API_UNAVAILABLE" }
    end

    local ok2, err2 = pcall(function() structure.addToGlobalStructureList() end)
    if not ok2 then
        return { success = false, error = "addToGlobalStructureList fallido: " .. tostring(err2), error_code = "CE_API_UNAVAILABLE" }
    end
...
function cmd_get_structure_by_name(params)
    ...
    if not ok then
        return { success = false, error = "getStructureCount fallido: " .. tostring(count), error_code = "CE_API_UNAVAILABLE" }
    end
    ...
    return { success = false, error = "Estructura no encontrada: " .. name, error_code = "NOT_FOUND" }
end
...
function cmd_add_element_to_structure(params)
    ...
    if not ename or offset == nil or not etype then
        return { success = false, error = "name, offset, tipo son requeridos", error_code = "INVALID_PARAMS" }
    end

    local vt = vartypeMap[string.lower(tostring(etype))]
    if not vt then
        return { success = false, error = "Tipo desconocido: " .. tostring(etype), error_code = "INVALID_PARAMS" }
    end

    local ok, element = pcall(function() return structure.addElement() end)
    if not ok or not element then
        return { success = false, error = "addElement fallido: " .. tostring(element), error_code = "CE_API_UNAVAILABLE" }
    end
    ...
    if not ok2 then
        return { success = false, error = "Error al establecer las propiedades del elemento: " .. tostring(err2), error_code = "CE_API_UNAVAILABLE" }
    end
...
function cmd_get_structure_elements(params)
    ...
    if not ok then
        return { success = false, error = "No se pudo leer el recuento de estructuras: " .. tostring(cnt), error_code = "CE_API_UNAVAILABLE" }
    end
...
local function unit18_get_al()
    ...
    if not ok or not al then
        return nil, { success = false, error = "No se puede obtener la AddressList", error_code = "CE_API_UNAVAILABLE" }
    end
    return al, nil
end
...
local function unit18_get_rec_by_id(al, id)
    ...
    if not ok or not rec then
        return nil, { success = false, error = "Registro de memoria no encontrado", error_code = "NOT_FOUND" }
    end
    return rec, nil
end
...
local function unit18_check_filename(filename)
    if type(filename) ~= "string" or filename == "" then
        return { success = false, error = "Nombre de archivo requerido", error_code = "INVALID_PARAMS" }
    end
    if filename:find("%.%.") then
        return { success = false, error = "No se permite el recorrido de la ruta", error_code = "INVALID_PARAMS" }
    end
end
...
function cmd_get_memory_record(params)
    ...
    if id == nil and desc == nil then
        return { success = false, error = "Id o descripción requerida", error_code = "INVALID_PARAMS" }
    end
    ...
    if not rec then
        return { success = false, error = "Registro de memoria no encontrado", error_code = "NOT_FOUND" }
    end
...
function cmd_create_memory_record(params)
    ...
    if type(description) ~= "string" or description == "" then
        return { success = false, error = "Descripción requerida", error_code = "INVALID_PARAMS" }
    end
    if type(address) ~= "string" or address == "" then
        return { success = false, error = "Dirección requerida", error_code = "INVALID_PARAMS" }
    end

    local vtName = UNIT18_TYPE_MAP[typeStr]
    if not vtName then
        return { success = false, error = "Tipo desconocido: " .. typeStr, error_code = "INVALID_PARAMS" }
    end
    ...
    local function set_prop(name, val)
        ...
        if not ok then
            ...
            return { success = false, error = "No se pudo establecer " .. name, error_code = "INTERNAL_ERROR" }
        end
    end
...
function cmd_delete_memory_record(params)
    local id = params.id
    if id == nil then
        return { success = false, error = "Id requerida", error_code = "INVALID_PARAMS" }
    end
...
function cmd_set_memory_record_value(params)
    ...
    if value == nil then
        return { success = false, error = "Valor requerido", error_code = "INVALID_PARAMS" }
    end
...
local function parse_xy(params)
    if params.x == nil then return nil, nil, "Parámetro faltante: x" end
    if params.y == nil then return nil, nil, "Parámetro faltante: y" end
    local x, y = tonumber(params.x), tonumber(params.y)
    if x == nil or y == nil then return nil, nil, "Los parámetros x e y deben ser números." end
    return x, y, nil
end

local function parse_vk(params)
    if params.vk == nil then return nil, "Parámetro faltante: vk (código de clave virtual de Windows, por ejemplo, 0x41 para 'A')" end
    local vk = tonumber(params.vk)
    if vk == nil then return nil, "El parámetro vk debe ser un número." end
    return vk, nil
end
...
local function run_key_action(fn, vk, fn_name)
    ...
    if not ok then return { success = false, error = fn_name .. " fallido: " .. tostring(err) } end
    return { success = true }
end
...
function cmd_get_pixel(params)
    ...
    if not ok then return { success = false, error = "getPixel fallido: " .. tostring(rgb) } end
...
function cmd_get_mouse_pos(params)
    ...
    if not ok then return { success = false, error = "getMousePos fallido: " .. tostring(x) } end
...
function cmd_set_mouse_pos(params)
    ...
    if not ok then return { success = false, error = "setMousePos fallido: " .. tostring(e) } end
    return { success = true }
end
...
function cmd_is_key_pressed(params)
    ...
    if not ok then return { success = false, error = "isKeyPressed fallido: " .. tostring(pressed) } end
...
function cmd_get_screen_info(params)
    ...
    if not ok_w then return { success = false, error = "getScreenWidth fallido: " .. tostring(width) } end
    if not ok_h then return { success = false, error = "getScreenHeight fallido: " .. tostring(height) } end
    if not ok_d then return { success = false, error = "getScreenDPI fallido: " .. tostring(dpi) } end
...
function cmd_find_window(params)
    ...
    if not title and not class_name then
        return { success = false, error = "Se debe proporcionar al menos uno de los títulos o class_name" }
    end
...
function cmd_show_message(params)
    local message = params.message
    if not message then
        return { success = false, error = "se requiere mensaje" }
    end
...
function cmd_show_selection_list(params)
    ...
    if type(options) ~= "table" then
        return { success = false, error = "Las opciones deben ser una lista de cadenas." }
    end
...
local function requireProcess()
    local pid = getOpenedProcessID()
    if not pid or pid == 0 then
        return false, { success = false, error = "No hay proceso adjunto", error_code = "NO_PROCESS" }
    end
    return true, nil
end
...
function cmd_aob_scan_unique(params)
    ...
    if not pattern then
        return { success = false, error = "No se proporciona ningún patrón", error_code = "INVALID_PARAMS" }
    end
    ...
    if not scanOk then
        return { success = false, error = "AOBScan fallido: " .. tostring(scanMsg), error_code = "SCAN_ERROR" }
    end
    ...
    if count ~= 1 then
        ...
        return {
            success    = false,
            error      = "Patrón coincidente " .. tostring(count) .. " veces (esperado 1)",
...
function cmd_aob_scan_module(params)
    ...
    if not pattern     then return { success = false, error = "No se proporciona ningún patrón",     error_code = "INVALID_PARAMS" } end
    if not module_name then return { success = false, error = "No se ha proporcionado ningún module_name", error_code = "INVALID_PARAMS" } end
    ...
    if not modBaseOk or not modBase or modBase == 0 then
        return { success = false, error = "Módulo no encontrado: " .. tostring(module_name), error_code = "INVALID_PARAMS" }
    end

    local modSizeOk = pcall(function() modSize = getModuleSize(module_name) end)
    if not modSizeOk or not modSize or modSize == 0 then
        return { success = false, error = "No se puede obtener el tamaño del módulo para: " .. tostring(module_name), error_code = "INVALID_PARAMS" }
    end
...
function cmd_aob_scan_module_unique(params)
    ...
    if count ~= 1 then
        return {
            success    = false,
            error      = "Patrón coincidente " .. tostring(count) .. " veces en el módulo (esperado 1)",
            error_code = "INVALID_PARAMS",
            count      = count
        }
    end
...
function cmd_pointer_rescan(params)
    ...
    if not value then
        return { success = false, error = "No se proporcionó ningún valor", error_code = "INVALID_PARAMS" }
    end
    ...
    if not rescanOk then
        return {
            success    = false,
            error      = "pointerRescan fallido: " .. tostring(rescanMsg),
            error_code = "SCAN_ERROR",
            note       = "Debe existir un escaneo de puntero previo en CE antes de llamar a pointer_rescan"
        }
    end

    return { success = true, result_count = -1, note = "Se completó la nueva exploración del puntero. Consulte la ventana CE Pointer Scanner para ver los resultados." }
end
...
function cmd_create_persistent_scan(params)
    ...
    local name = params.name
    if not name or name == "" then
        return { success = false, error = "No se proporcionó ningún nombre", error_code = "INVALID_PARAMS" }
    end
    ...
    if not msOk or not ms then
        return { success = false, error = "createMemScan fallido: " .. tostring(msMsg), error_code = "SCAN_ERROR" }
    end
...
function cmd_persistent_scan_first_scan(params)
    ...
    if not name  then return { success = false, error = "No se proporcionó ningún nombre",  error_code = "INVALID_PARAMS" } end
    if not value then return { success = false, error = "No se proporcionó ningún valor", error_code = "INVALID_PARAMS" } end

    local entry = serverState.persistent_scans[name]
    if not entry then
        return { success = false, error = "Escanear '" .. name .. "' no encontrado. Llama primero a create_persistent_scan.", error_code = "INVALID_PARAMS" }
    end
    ...
    if not fsOk then
        return { success = false, error = "firstScan fallido: " .. tostring(fsMsg), error_code = "SCAN_ERROR" }
    end
    ...
    if not flOk then
        return { success = false, error = "createFoundList fallido: " .. tostring(flMsg), error_code = "SCAN_ERROR" }
    end
...
function cmd_persistent_scan_next_scan(params)
    ...
    local entry = serverState.persistent_scans[name]
    if not entry then
        return { success = false, error = "Escanear '" .. name .. "' no encontrado.", error_code = "INVALID_PARAMS" }
    end
    if not entry.has_scan then
        return { success = false, error = "No se realizó el primer escaneo para '" .. name .. "'. Llame primero a persistent_scan_first_scan.", error_code = "INVALID_PARAMS" }
    end
    ...
    if not nsOk then
        return { success = false, error = "nextScan fallido: " .. tostring(nsMsg), error_code = "SCAN_ERROR" }
    end
...
function cmd_persistent_scan_get_results(params)
    ...
    if not entry.fl then
        return { success = false, error = "No hay resultados para '" .. name .. "'. Ejecute first_scan primero.", error_code = "INVALID_PARAMS" }
    end
...
local function sanitizeFilename(f)
    if type(f) ~= "string" or f:find("%.%.") then return nil, "Nombre de archivo no válido" end
    return f, nil
end

function cmd_copy_memory(params)
    local pid = getOpenedProcessID()
    if not pid or pid == 0 then return { success = false, error = "No hay proceso adjunto" } end
    ...
    if not src then return { success = false, error = "Dirección de origen faltante" } end
    if not size or size <= 0 then return { success = false, error = "Faltante o tamaño no válida" } end

    if type(src) == "string" then src = getAddressSafe(src) end
    if not src then return { success = false, error = "Dirección de origen no válida" } end

    local destAddr = nil
    if dest ~= nil then
        if type(dest) == "string" then destAddr = getAddressSafe(dest)
        else destAddr = dest end
        if not destAddr then return { success = false, error = "Dirección de destino no válida" } end
    end

    local ok, result = pcall(copyMemory, src, size, destAddr, method)
    if not ok or not result then
        return { success = false, error = "copyMemory fallido: " .. tostring(result) }
    end
...
function cmd_compare_memory(params)
    ...
    if not addr1 then return { success = false, error = "Faltante addr1" } end
    if not addr2 then return { success = false, error = "Faltante addr2" } end
    if not size or size <= 0 then return { success = false, error = "Faltante o tamaño no válida" } end

    if type(addr1) == "string" then addr1 = getAddressSafe(addr1) end
    if type(addr2) == "string" then addr2 = getAddressSafe(addr2) end
    if not addr1 then return { success = false, error = "Inválido addr1" } end
    if not addr2 then return { success = false, error = "Inválido addr2" } end

    local ok, r1, r2 = pcall(compareMemory, addr1, addr2, size, method)
    if not ok then
        return { success = false, error = "compareMemory fallido: " .. tostring(r1) }
    end
...
function cmd_write_region_to_file(params)
    ...
    if not addr then return { success = false, error = "Dirección faltante" } end
    if not size or size <= 0 then return { success = false, error = "Faltante o tamaño no válida" } end
    ...
    if not ok then
        return { success = false, error = "writeRegionToFile fallido: " .. tostring(bytes_written) }
    end
...
function cmd_read_region_from_file(params)
    ...
    if not destination then return { success = false, error = "Dirección de destino faltante" } end

    if type(destination) == "string" then destination = getAddressSafe(destination) end
    if not destination then return { success = false, error = "Dirección de destino no válida" } end

    local ok, bytes_read = pcall(readRegionFromFile, sanitized, destination)
    if not ok then
        return { success = false, error = "readRegionFromFile fallido: " .. tostring(bytes_read) }
    end
...
function cmd_md5_memory(params)
    ...
    if not ok or not result then
        return { success = false, error = "md5memory fallido: " .. tostring(result) }
    end
...
function cmd_md5_file(params)
    ...
    if not ok or not result then
        return { success = false, error = "md5file fallido: " .. tostring(result) }
    end
...
function cmd_create_section(params)
    ...
    if not ok or not handle then
        return { success = false, error = "createSection fallido: " .. tostring(handle) }
    end
...
function cmd_map_view_of_section(params)
    ...
    if not handle then return { success = false, error = "handle faltante" } end

    if type(handle) == "string" then handle = tonumber(handle, 16) end
    if not handle then return { success = false, error = "Identificador no válido" } end

    local prefAddr = nil
    if address ~= nil then
        if type(address) == "string" then prefAddr = getAddressSafe(address)
        else prefAddr = address end
        if not prefAddr then return { success = false, error = "Dirección no válida" } end
    end
    ...
    if not ok or not mapped then
        return { success = false, error = "mapViewOfSection fallido: " .. tostring(mapped) }
    end
...
function cmd_register_symbol(params)
    ...
    if type(name) ~= "string" or name == "" then
        return { success = false, error = "El parámetro 'name' debe ser una cadena que no esté vacía", error_code = "INVALID_PARAMS" }
    end
    if type(address) ~= "string" and type(address) ~= "number" then
        return { success = false, error = "El parámetro 'address' debe ser una cadena o un número entero", error_code = "INVALID_PARAMS" }
    end
    ...
    if not resolvedAddr or resolvedAddr == 0 then
        return { success = false, error = "Dirección no válida: " .. tostring(address), error_code = "INVALID_ADDRESS" }
    end
    local ok, err = pcall(registerSymbol, name, resolvedAddr, do_not_save)
    if not ok then
        return { success = false, error = "registerSymbol fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
...
function cmd_unregister_symbol(params)
    ...
    if not ok then
        return { success = false, error = "unregisterSymbol fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
    return { success = true }
end
...
function cmd_enum_registered_symbols(params)
    ...
    if not ok then
        return { success = false, error = "enumRegisteredSymbols fallido: " .. tostring(result), error_code = "INTERNAL_ERROR" }
    end
    local symbols = {}
...
function cmd_delete_all_registered_symbols(params)
    ...
    if not ok then
        return { success = false, error = "deleteAllRegisteredSymbols fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
...
function cmd_enable_windows_symbols(params)
    ...
    if not ok then
        return { success = false, error = "enableWindowsSymbols fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
    return { success = true }
end
...
function cmd_enable_kernel_symbols(params)
    ...
    if not ok then
        ...
        if errMsg:lower():find("dbk") or errMsg:lower():find("kernel") or errMsg:lower():find("driver") then
            return { success = false, error = "Controlador kernel no cargado", error_code = "DBK_NOT_LOADED" }
        end
        return { success = false, error = "enableKernelSymbols fallido: " .. errMsg, error_code = "INTERNAL_ERROR" }
    end
    return { success = true }
end
...
function cmd_get_symbol_info(params)
    ...
    if not ok then
        return { success = false, error = "getSymbolInfo fallido: " .. tostring(info), error_code = "INTERNAL_ERROR" }
    end
    if not info then
        return { success = false, error = "Símbolo no encontrado: " .. name, error_code = "NOT_FOUND" }
    end
...
function cmd_get_module_size(params)
    ...
    if type(module_name) ~= "string" or module_name == "" then
        return { success = false, error = "El parámetro 'module_name' debe ser una cadena que no esté vacía", error_code = "INVALID_PARAMS" }
    end
    local ok, sz = pcall(getModuleSize, module_name)
    if not ok then
        return { success = false, error = "getModuleSize fallido: " .. tostring(sz), error_code = "INTERNAL_ERROR" }
    end
    if not sz then
        return { success = false, error = "Módulo no encontrado: " .. module_name, error_code = "NOT_FOUND" }
    end
...
function cmd_load_new_symbols(params)
    ...
    if not ok then
        return { success = false, error = "loadNewSymbols fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
    return { success = true }
end
...
function cmd_reinitialize_symbol_handler(params)
    ...
    if not ok then
        return { success = false, error = "reinitializeSymbolhandler fallido: " .. tostring(err), error_code = "INTERNAL_ERROR" }
    end
    return { success = true }
end
-- >>> END UNIT-12 <<<
...
function cmd_assemble_instruction(params)
    ...
    if not line or line == "" then
        return { success = false, error = "No se proporciona línea de instrucciones" }
    end

    if type(address) == "string" then address = getAddressSafe(address) end
    if address == nil and params.address ~= nil then
        return { success = false, error = "Dirección no válida: " .. tostring(params.address) }
    end
    ...
    if not asmOk then
        return { success = false, error = "assemble() error planteado: " .. tostring(result) }
    end

    if not result then
        return { success = false, error = "assemble() devolvió nulo (instrucción o dirección no válida)" }
    end
...
function cmd_auto_assemble_check(params)
    ...
    if not script or script == "" then
        return { success = false, error = "No se proporciona ningún script" }
    end
...
function cmd_compile_c_code(params)
    ...
    if not source or source == "" then
        return { success = false, error = "No se proporciona código fuente" }
    end

    if type(compile) ~= "function" then
        return { success = false, error = "Compilador TCC no disponible", error_code = "CE_API_UNAVAILABLE" }
    end
...
function cmd_compile_cs_code(params)
    ...
    if type(compileCS) ~= "function" then
        return { success = false, error = ".NET runtime o compileCS No disponible", error_code = "CE_API_UNAVAILABLE" }
    end
    ...
    if not result then
        return { success = false, assembly_handle = nil, error = "compileCS devolvió nulo" }
    end
...
function cmd_generate_api_hook_script(params)
    ...
    if not address then return { success = false, error = "No se proporcionó ninguna dirección" } end
    if not targetAddress then return { success = false, error = "No se ha proporcionado target_address" } end

    if type(address) == "string" then address = getAddressSafe(address) end
    if type(targetAddress) == "string" then targetAddress = getAddressSafe(targetAddress) end

    if not address then return { success = false, error = "Dirección no válida: " .. tostring(params.address) } end
    if not targetAddress then return { success = false, error = "Inválido target_address: " .. tostring(params.target_address) } end
    ...
    if not genOk then
        return { success = false, error = "generateAPIHookScript fallido: " .. tostring(result) }
    end

    if not result then
        return { success = false, error = "generateAPIHookScript devolvió nulo" }
    end
...
function cmd_generate_code_injection_script(params)
    ...
    if not genOk then
        sl.destroy()
        return { success = false, error = "generateCodeInjectionScript fallido: " .. tostring(genErr) }
    end
    ...
    if not script or script == "" then
        return { success = false, error = "generateCodeInjectionScript produjo un script vacío" }
    end
...
function cmd_debug_get_context(params)
    ...
    if not callOk then
        return { success = false, error = "debug_getContext fallido: " .. tostring(callErr), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_set_context(params)
    ...
    if type(registers) ~= "table" then
        return { success = false, error = "los registros deben ser un object/dict", error_code = "INVALID_PARAMS" }
    end
    ...
    if not setOk then
        return { success = false, error = "debug_setContext fallido: " .. tostring(setErr), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_get_xmm_pointer(params)
    ...
    if not ptrOk then
        return { success = false, error = "debug_getXMMPointer fallido: " .. tostring(ptr), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_set_last_branch_recording(params)
    ...
    if iface ~= 3 then
        return {
            success            = false,
            error              = "LBR requiere un depurador del kernel",
            error_code         = "CE_API_UNAVAILABLE",
    ...
    if not lbrOk then
        return { success = false, error = "debug_setLastBranchRecording fallido: " .. tostring(lbrErr), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_get_last_branch_record(params)
    ...
    if not recOk then
        return { success = false, error = "debug_getLastBranchRecord fallido: " .. tostring(record), error_code = "CE_API_ERROR" }
    end

    if type(record) ~= "table" then
        return { success = false, error = "Regreso inesperado de debug_getLastBranchRecord: " .. tostring(record), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_set_breakpoint_for_thread(params)
    ...
    if not threadId then return { success = false, error = "thread_id es requerido", error_code = "INVALID_PARAMS" } end

    local ok, err = u11_guard()
    if not ok then return err end

    if type(addr) == "string" then addr = getAddressSafe(addr) end
    if not addr then return { success = false, error = "Dirección no válida", error_code = "INVALID_PARAMS" } end
    ...
    if not setOk then
        ...
        return { success = false, error = "debug_setBreakpointForThread fallido: " .. tostring(setErr), error_code = "CE_API_ERROR" }
    end
...
function cmd_debug_remove_breakpoint_for_thread(params)
    ...
    if not remOk then
        return { success = false, error = "debug_removeBreakpoint fallido: " .. tostring(remErr), error_code = "CE_API_ERROR" }
    end
...
-- ============================================================================
-- MANEJADORES DE COMANDOS - CONTROL DEL DEPURADOR (Unidad 10)
-- ============================================================================
...
local function requireDebugger()
    ...
    if not ok or not isDbg then
        return { success = false, error = "El depurador no está adjunto" }
    end
end
...
function cmd_debug_break_thread(params)
    ...
    if not tid then
        return { success = false, error = "Falta el parámetro requerido: thread_id" }
    end
...
function cmd_debug_continue(params)
    ...
    elseif method == "step_over" then
        ...
        return { success = false, error = "Método desconocido: " .. tostring(method) .. ". Válido: run, step_into, step_over" }
    end
...
function cmd_inject_dll(params)
    if not requireProcess() then return { success = false, error = "No hay proceso adjunto" } end
    local filepath = params.filepath
    if not filepath then return { success = false, error = "No se proporcionó ninguna filepath" } end
    local skip = params.skip_symbol_reload or false

    local ok, result = pcall(injectDLL, filepath, skip)
    if not ok then
        return { success = false, error = "injectDLL fallido: " .. tostring(result) }
    end
...
function cmd_inject_dotnet_dll(params)
    ...
    if not dllpath    then return { success = false, error = "No se proporcionó ninguna filepath" } end
    if not className  then return { success = false, error = "No se ha proporcionado ningún class_name" } end
    if not methodName then return { success = false, error = "No se ha proporcionado ningún method_name" } end

    local ok, result = pcall(injectDotNetDLL, dllpath, className, methodName, param, timeout)
    if not ok then
        return { success = false, error = "injectDotNetDLL fallido: " .. tostring(result) }
    end
...
function cmd_execute_code(params)
    ...
    if not ok then
        return { success = false, error = "executeCode fallido: " .. tostring(retval) }
    end
...
function cmd_execute_code_ex(params)
    ...
    if not ok then
        return { success = false, error = "executeCodeEx fallido: " .. tostring(retval) }
    end
    return { success = true, return_value = retval }
end
...
function cmd_execute_method(params)
    ...
    if not ok then
        return { success = false, error = "executeMethod fallido: " .. tostring(retval) }
    end
    return { success = true, return_value = retval }
end
...
function cmd_execute_code_local(params)
    ...
    if not ok then
        return { success = false, error = "executeCodeLocal fallido: " .. tostring(retval) }
    end
    return { success = true, return_value = retval }
end
...
function cmd_execute_code_local_ex(params)
    ...
    if not ok then
        return { success = false, error = "executeCodeLocalEx fallido: " .. tostring(retval) }
    end
    return { success = true, return_value = retval }
end
...
function cmd_allocate_memory(params)
    ...
    if not size or type(size) ~= "number" or size <= 0 then
        return { success = false, error = "Parámetro de tamaño no válido", error_code = "INVALID_PARAMS" }
    end
    ...
    if not protConst then
        return { success = false, error = "Cadena de protección no válida; use r, rw, rx, or rwx", error_code = "INVALID_PARAMS" }
    end
    ...
    if not result or result == 0 then
        return { success = false, error = "La asignación devolvió una dirección nula", error_code = "OUT_OF_RESOURCES" }
    end
...
function cmd_free_memory(params)
    ...
    if not addr or addr == 0 then
        return { success = false, error = "Dirección no válida", error_code = "INVALID_ADDRESS" }
    end
...
function cmd_allocate_shared_memory(params)
    ...
    local name = params.name
    if not name or name == "" then
        return { success = false, error = "Parámetro de nombre no válido", error_code = "INVALID_PARAMS" }
    end
    ...
    if not result or result == 0 then
        return { success = false, error = "La asignación de memoria compartida devolvió una dirección nula", error_code = "OUT_OF_RESOURCES" }
    end
...
function cmd_allocate_kernel_memory(params)
    ...
    if not dbk_initialized() then
        return { success = false, error = "Controlador kernel (DBK) no cargado", error_code = "DBK_NOT_LOADED" }
    end
    ...
    if not result or result == 0 then
        return { success = false, error = "La asignación del kernel devolvió una dirección nula", error_code = "OUT_OF_RESOURCES" }
    end
...
function cmd_open_process(params)
    ...
    if not target then return { success = false, error = "Faltante process_id_or_name" } end
    ...
    if not ok2 or not pid or pid == 0 then
        return { success = false, error = "Proceso no encontrado o no se pudo abrir" }
    end
...
function cmd_get_processid_from_name(params)
    local name = params.name
    if not name then return { success = false, error = "Nombre faltante" } end

    local ok, pid = pcall(getProcessIDFromProcessName, name)
    if not ok then return { success = false, error = tostring(pid) } end
    if not pid or pid == 0 then
        return { success = false, error = "Proceso no encontrado", error_code = "NOT_FOUND" }
    end
...
function cmd_create_process(params)
    ...
    if not path then return { success = false, error = "Ruta faltante" } end
    local args = params.args or ""
...
local commandHandlers = {
    ...
    -- Salida de depuración y multimedia (Unidad 23)
    ...
    -- Unidad-21: Modo Kernel / Extensiones DBVM
    ...
    -- Ejecución de Shell (UNIT-20b): puerta de seguridad aplicada en el lado de Python
    ...
    -- >>> COMENZAR UNIDAD-19 entradas del despachador <<<
    ...
    -- >>> UNIDAD FINAL-19 <<<
    -- >>> COMENZAR UNIDAD-18 entradas del despachador <<<
    ...
    -- >>> UNIDAD FINAL-18 <<<
    -- Automatización de entrada (Unidad-17): en todo el sistema, no se requiere protección de proceso
    ...
    -- Operaciones de memoria (Unidad 14)
    ...
    -- Montaje y compilación (Unidad 13)
    ...
    -- >>> COMENZAR UNIDAD-12 entradas del despachador <<<
    ...
    -- >>> UNIDAD FINAL-12 <<<
    -- Unidad 11: Contexto de depuración + puntos de interrupción por subproceso
    ...
    -- Control del depurador (Unidad 10)
    ...
    -- Inyección y ejecución de código (Unidad-09)
    ...
    -- >>> COMENZAR UNIDAD-08 entradas del despachador <<<
...
-- ============================================================================
-- PROCESADOR DE COMANDO PRINCIPAL
-- ============================================================================
...
local function executeCommand(jsonRequest)
    ...
    if not handler then
        return json.encode({ jsonrpc = "2.0", error = { code = -32601, message = "Método no encontrado: " .. tostring(method) }, id = id })
    end
    
    local ok2, result = pcall(handler, params)
    if not ok2 then
        return json.encode({ jsonrpc = "2.0", error = { code = -32603, message = "Error interno: " .. tostring(result) }, id = id })
    end
...
-- ============================================================================
-- SERVIDOR DE PIPE BASADO EN HILOS (GUI SIN BLOQUEO)
-- ============================================================================
-- Reemplaza la arquitectura del temporizador v10 para evitar que la GUI se congele.
-- La E/S ocurre en Worker Thread. La ejecución ocurre en el hilo principal.

local function PipeWorker(thread)
    log("Hilo de trabajo iniciado: esperando conexión...")
    
    while not thread.Terminated do
        -- Crear instancia de tubería por intento de conexión
        -- Aumento del tamaño del búfer a 256 KB para un mejor rendimiento
        local pipe = createPipe(PIPE_NAME, 262144, 262144)  -- Búfer de 256 KB (en lugar de 64 KB)
        if not pipe then
            log("Fatal: No se pudo crear la tubería")
            return
        end
        
        -- Almacenar la referencia para que podamos destruirla desde el hilo principal (stopServer) para romper las llamadas de bloqueo.
        serverState.workerPipe = pipe
        
        -- tiempo de espera para operaciones de bloqueo (conectar/leer)
        -- NO configuramos pipe.Timeout porque se desconecta automáticamente cuando se agota el tiempo de espera.
        -- Confiamos en bloquear lecturas y pipe.destroy() de stopServer para romper el bloqueo.
        -- pipe.Timeout = 0 (predeterminado, infinito)
        
        -- Espere al cliente (Bloqueando, pero en el hilo, por lo que la GUI está bien)
        -- LuaPipeServer usa AcceptConnection().
        -- Nota: Es posible que AcceptConnection no devuelva un valor booleano, por lo que verificamos pipe.Connected después.
        
        -- log("Subproceso: Llamando a AcceptConnection()...")
        pcall(function()
        ...
        if pipe.Connected and not thread.Terminated then
            ...
            while not thread.Terminated and pipe.Connected do
                -- Intenta leer el encabezado (4 bytes)
                -- Usamos pcall para manejar tiempos de espera/errores con elegancia
                ...
                if ok and lenBytes and #lenBytes == 4 then
                    ...
                    -- Longitud del control de cordura
                    if len > 0 and len < 32 * 1024 * 1024 then
                        local payload = pipe.readString(len)
                        
                        if payload then
                            -- CRÍTICO: EJECUTAR EN EL HILO PRINCIPAL
                            -- Pausamos al trabajador y ejecutamos la lógica en el hilo de la GUI para estar seguros.
                            local response = nil
                            thread.synchronize(function()
                                response = executeCommand(payload)
                            end)
                            
                            -- Escribir respuesta (tema del trabajador)
                            if response then
                                local rLen = #response
                                local b1 = rLen % 256
                             ...
                             -- log("Subproceso: Error al leer la carga útil (nulo)")
                        end
                    end
                else
                    -- Error de lectura. Si se desconecta la tubería, el circuito terminará en la próxima revisión.
                    if not pipe.Connected then
                        -- Cliente desconectado elegantemente
                    end
                end
            end
            ...
            -- Depuración: se devolvió AcceptConnection pero la tubería no es válida
            -- Esto suele suceder al finalizar o en un estado extraño.
            if not thread.Terminated then
                -- log("Subproceso: Registro auxiliar: intento de conexión no válido")
            end
        end
        ...
        -- Dormir brevemente antes de recrear la tubería para aceptar una nueva conexión.
        if not thread.Terminated then sleep(50) end
    end
    
    log("Hilo de trabajo terminado")
end
...
function StopMCPBridge()
    if serverState.workerThread then
        log("Deteniendo el servidor (Terminando el hilo)...")
        serverState.workerThread.terminate()
        
        -- Forzar la destrucción de la tubería si actualmente está bloqueada en aceptar conexión o leer
    ...
    -- CRÍTICO: Limpiar todos los recursos zombies (puntos de interrupción, vigilancias DBVM, escaneos)
    cleanupZombieState()
...
function StartMCPBridge()
    StopMCPBridge()  -- Esto ahora también llama a cleanupZombieState()
    
    -- Actualizar estado global
    log("Iniciar MCP Bridge v" .. VERSION)
    
    serverState.running = true
    serverState.connected = false
    
    -- Crear el hilo de trabajo
    serverState.workerThread = createThread(PipeWorker)
    
    log("===========================================")
    log("MCP Server Escuchando: " .. PIPE_NAME)
    log("Arquitectura: Threaded I/O + Ejecución sincronizada")
    log("Limpieza: Prevención zombi activa")
    log("===========================================")
end
